...
bool _wrap = true;
uint8_t _colstart = 0, _rowstart = 0, _tft_type, _rotation = 0, _xstart = 0,
        _ystart = 0;

// we keept this public
uint8_t tft_width = 240, tft_height = 240;
...
void fillScreen(uint16_t color) { fillRectangle(0, 0, _width, _height, color); }
...
void drawChar(uint8_t x, uint8_t y, uint8_t c, uint16_t color, uint16_t bg,
              uint8_t size) {
    int8_t i, j;
    if ((x >= _width) || (y >= _height)) return;
    if (size < 1) size = 1;
...
void drawText(uint8_t x, uint8_t y, const char *_text, uint16_t color,
...
void setScrollDefinition(uint8_t top_fix_height, uint8_t bottom_fix_height,
...
void NormalDisplay() { write_command(ST7735_NORON); }
...
#if defined TFT_ENABLE_FONTS
GFXfont *_gfxFont;
void setFont(const GFXfont *f) { _gfxFont = (GFXfont *)f; }
...
bool bmpDraw(int8_t x, int8_t y, int8_t *bmpname) {
    bool ec = 0, padding = 0;
    int8_t bmpdata[pixel_buffer * 3], planes, depth, r, g, b, col, row;
    ...
    if (fat16_open_file(bmpname) != 0) return false;
    ...
    if (ec != 0 || compression != 0 || depth != 24 || planes != 1) return false;
    ...
    if ((x + bmpWidth - 1) >= _width) {  // _width is the TFT screen width
        ...
        if (bmpWidth % pixel_buffer) padding_factor++;
        padding_factor *= buffer;
    }
    if ((y + bmpHeight - 1) >= _height) {  // _height is the TFT screen height
        offset += row_size * (bmpHeight - _height +
                              y);  // Only upper part will be displayed
        bmpHeight = _height - y;
    }
    file_pointer = offset;
    ...
    if (_tft_type != 2) {
        ...
        write_command(ST7735_MADCTL);
        if (_tft_type == 0) write_data(0x48);
        if (_tft_type == 1) write_data(0x40);
    }
    ...
    if (_tft_type != 2) {
        ...
        write_command(ST7735_MADCTL);
        if (_tft_type == 0) write_data(0xC8);
        if (_tft_type == 1) write_data(0xC0);
    }
    // Good BMP
    return true;
}
